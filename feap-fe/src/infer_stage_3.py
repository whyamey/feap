...
def plot_similarity_histogram(same_class_sims, diff_class_sims, same_class_hamming, diff_class_hamming, save_path):
    plt.clf()
    plt.close('all')
    ...
    ax2.hist(same_class_hamming, bins=50, alpha=0.5, 
             label=f'Same Class (μ={same_class_hamming.mean():.3f}, σ={same_class_hamming.std():.3f})',
             density=True, color='blue')
    ax2.hist(diff_class_hamming, bins=50, alpha=0.5, 
...
def main():
    ...
    classes = [d for d in os.listdir(config['data_dir']) if os.path.isdir(os.path.join(config['data_dir'], d))]
    ...
    original_checkpoint = torch.load('checkpoint_epoch_1000.pt', map_location=device)
    refinement_checkpoint = torch.load('checkpoint_epoch_421000.pt', map_location=device)
    ...
    for class_id in tqdm(os.listdir(config['data_dir'])):
        ...
        for img_name in os.listdir(class_dir):
            ...
            bright_down_flip = cv2.convertScaleAbs(cv2.flip(image, 1), alpha=0.9, beta=0)
            image_variations.extend([bright_up, bright_down, bright_down_flip, bright_up_flip])
            ...
            offset = width * 0.015
            dst_points = np.float32([[0+offset, 0], [width-1-offset, 0], 
                        [0, height-1], [width-1, height-1]])
            transform_matrix = cv2.getPerspectiveTransform(src_points, dst_points)
            warped_img = cv2.warpPerspective(image, transform_matrix, (width, height))
            warped_img_f = cv2.warpPerspective(cv2.flip(image, 1), transform_matrix, (width, height))
            ...
            translated_f = cv2.warpAffine(cv2.flip(image, 1), translation_matrix, (width, height))
