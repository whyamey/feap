...
class IrisDataset(Dataset):
    def __init__(self, root_dir: str, transform=None, class_filter=None):
        ...
        self.class_to_idx = {cls_name: idx for idx, cls_name in enumerate(sorted(class_folders))}
...
def plot_similarity_histogram(same_class_sims, diff_class_sims, embeddings, labels, epoch, save_dir='histograms_refined'):
    plt.clf()
    plt.close('all')
    ...
    ax1.hist(same_class_sims, bins=50, alpha=0.5, 
             label=f'Same Class (μ={same_class_sims.mean():.3f}, σ={same_class_sims.std():.3f})',
             density=True, color='blue')
    ax1.hist(diff_class_sims, bins=50, alpha=0.5, 
    ...
    ax2.hist(same_hamming, bins=50, alpha=0.5, 
             label=f'Same Class (μ={same_hamming.mean():.3f}, σ={same_hamming.std():.3f})',
             density=True, color='blue')
    ax2.hist(diff_hamming, bins=50, alpha=0.5, 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'distribution_hist_epoch_{epoch+1:04d}.png'), 
                bbox_inches='tight')
    plt.close()
...
def extract_and_train():
    ...
    class_folders = [d for d in os.listdir(config['train_dir']) if os.path.isdir(os.path.join(config['train_dir'], d))]
    num_classes = len(class_folders)
    print(f"Found {num_classes} classes in training directory")
    
    original_model = IrisNet(num_classes=num_classes, embedding_dim=config['embedding_dim']).to(device)
    checkpoint = torch.load('checkpoint_epoch_1000.pt', map_location=device)
    ...
    hist_loader = DataLoader(hist_dataset, batch_size=config['batch_size'],
                           shuffle=False, num_workers=4, pin_memory=True)
    ...
    for epoch in range(config['epochs']):
        ...
        for i in pbar:
            ...
            loss, metrics = criterion(refined, batch_labels, refinement_model.temperature)
        ...
        if (epoch + 1) % 500 == 0:
            ...
            with torch.no_grad():
                ...
                plot_similarity_histogram(same_class_sims, diff_class_sims, refined_hist, hist_labels, epoch)
        ...
        if (epoch + 1) % 500 == 0:
            ...
            print(f"Same Class Mean: {same_class_sims.mean():.3f}, Std: {same_class_sims.std():.3f}")
            print(f"Diff Class Mean: {diff_class_sims.mean():.3f}, Std: {diff_class_sims.std():.3f}")
