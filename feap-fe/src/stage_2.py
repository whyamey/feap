...
def get_similarity_matrix(embeddings: torch.Tensor, labels: torch.Tensor) -> tuple:
    ...
    for i in range(n):
        ...
        if len(curr_negs) < max_neg_count:
            padding = torch.full((max_neg_count - len(curr_negs),), -1.0, 
...
def plot_similarity_histogram(same_class_sims, diff_class_sims, epoch, save_dir='histograms'):
    os.makedirs(save_dir, exist_ok=True)
    
    plt.figure(figsize=(10, 6))
    
    plt.hist(same_class_sims, bins=50, alpha=0.5, label=f'Same Class (μ={same_class_sims.mean():.3f}, σ={same_class_sims.std():.3f})',
             density=True, color='blue')
    plt.hist(diff_class_sims, bins=50, alpha=0.5, label=f'Different Class (μ={diff_class_sims.mean():.3f}, σ={diff_class_sims.std():.3f})',
...
def create_train_val_split(dataset_path, val_ratio=0.2):
    ...
    train_classes, val_classes = train_test_split(classes, test_size=val_ratio, random_state=42)
...
def train_iris_model(config, resume_training=False, checkpoint_path='checkpoints/start_here.pt'):
    ...
    train_dataset = IrisDataset(config['iitd_left_dir'], 
                               transform=transform,
                               class_filter=split_info['train_classes'])
    train_dataset = AugmentedIrisDataset(train_dataset)
    
    val_dataset = IrisDataset(config['iitd_left_dir'],
                             transform=transform,
    ...
    model = IrisNet(num_classes=train_dataset.dataset.get_num_classes(),
